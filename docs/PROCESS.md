...
The last row is where it goes wrong. On a small team these roles collapse into one person, and
once they have collapsed the author is reviewing their own assumptions and the tests are written
to agree with the code. So they run as separate passes, in order, with separate outputs. A pass
that produces no artifact did not happen.
...
that the tests reference the symbols. It says nothing about whether any individual test would
catch a *wrong* implementation, which is the thing worth knowing. That is the `absence as
evidence` anti-pattern in a different hat, and an earlier wording of this section committed it.
...
At minimum, one mutation per class of defect the unit can have. For a lattice those classes are
under-approximation, matcher, normalisation, lattice law, determinism and rendering. Add one
...
"Working" is the floor. The target is code a senior engineer reads on a Monday morning and has
nothing to say about. Every layer: engine, CLI, API, schema, frontend, build, docs.
...
The test has two halves: **nothing can be deleted without losing meaning, and nothing can be
added that would gain anything.** Until both hold, it is not finished.
...
"It looks fine" is not a standard. No arbitrary pixel value where a token exists. No component
that works at only one width. No animation without a `prefers-reduced-motion` path. No colour
...
On every unit of work, all of these happen, and they are separate acts even when one person
performs them:

- **Architecture.** What is the shape, what are the boundaries, what could be wrong later.
- **Implementation.** The code, whole, with nothing stubbed.
...
- **Review.** Semantics read first, code second, looking for one place they disagree. Use a
  reader who has not seen the implementation reasoning where the unit is non-trivial.
...
Review and adversarial QA are done by someone other than the implementer. Run things twice. Read the upstream source
rather than assuming its behaviour.
...
**Stop and ask only for the triggers in `When to stop and ask`, plus:**

- Rewriting published history, or any force-push.
- Deleting or renaming something outside the current unit's file list.
- Adding a dependency. The licence rule under `Dependencies` is absolute.
- A change to `docs/PROCESS.md` or `docs/ENGINEERING.md`. Those are the Product owner's files. Propose the edit and the reasoning; do not apply it.
...
<n> calls made, and the <m> that were denied."*
...
If you need the tree clean and it is not, do not stash. Ask, or copy the repository elsewhere
and work there.
...
and the question is what changed. By then the entry either exists or it does not.

...
GitHub changed its subject grammar in July 2026, and AWS added five condition keys in February.
Every rule-matching tool in this space is broken because of it. A lattice is not.
...
web/                   WASM explorer.
testdata/              Golden corpus, one directory per document kind. Every case has a
                       rationale file. The issuer registry is the module
                       github.com/CloudArq-net/issuers, imported, never copied.
```

**`internal/eval`, `internal/parse`, `internal/trust`, `internal/join` and `internal/registry`
must not import anything that does IO.** `test/arch/purity_test.go` enforces it and fails on
`net`, `os`, `math/rand`, or any cloud SDK. `time` is allowed for `time.Time` as a value and
never for the clock.
...
treated as terminal. A blanket transitive rule would be unworkable rather than strict:
`encoding/json` reaches `os` through `fmt`, so the naive version rejects `evidence` and every
parser we will ever write. The allowlist is the narrowest thing that keeps the rule meaningful,
and adding to it is a deliberate act that belongs in a commit message.
...
Competitors hardcode issuer hostnames in source. Prowler's `MULTI_TENANT_OIDC_ISSUER_HOSTS` is
a four-element Python set, so every new issuer costs them a release.

**A new issuer, claim key, or subject grammar must be addable by a release of the issuers
module and nothing else.** If adding support for a new issuer requires a code change, the abstraction
...
- **100% statement coverage on `internal/eval`, `internal/trust` and `internal/parse`, enforced
  by `make cover`. Nowhere else.** Blanket coverage targets produce test theatre. The evaluator
  and the readers that feed it are where a wrong answer is silent and consequential.
...
- **Conformance vectors from an external project, if any are vendored.** Treat a disagreement
...
- **It never holds a cloud credential.** It ingests findings JSON. This one constraint is the
  entire procurement story, and there is no price at which violating it is worth it.
- **Ingest is idempotent**: `POST /v1/scans {sha256, findings}` → 201 first time, 200 with the
  same id thereafter. CI is flaky and retries. A non-idempotent ingest produces duplicate scans,
...
- **The watcher is keyed on the counterparty, never on the customer.** `counterparty` is a
  ...
  fan out to every subscribed tenant. Outbound volume scales with distinct counterparties rather
  than with customers, so the hundredth customer costs nothing to watch and fleet intelligence
  falls out of the schema instead of being bolted on.
...
- Empty states teach: "no external principals found — here are the <n> calls made, and the <m>
  that were denied."
...
keystrokes. At 100ms it feels like a form. At 16ms it feels like an
